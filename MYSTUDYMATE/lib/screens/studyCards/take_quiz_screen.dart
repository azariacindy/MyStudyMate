...
class _TakeQuizScreenState extends State<TakeQuizScreen> {
  ...
  late Map<int, List<int>>
  _answerMappings; // questionIndex -> [shuffled answer indices]

  dynamic get _currentQuestion => _shuffledQuestions[_currentQuestionIndex];
  ...
  /// Prepare and shuffle questions and answers
  void _prepareQuestions() {
    final questions = List<dynamic>.from(widget.quizData['questions'] ?? []);

    // Shuffle questions
    questions.shuffle();
    _shuffledQuestions = questions;

    // Shuffle answers for each question and keep track of mappings
    _answerMappings = {};
    for (int i = 0; i < _shuffledQuestions.length; i++) {
      ...
      List<int> indices = List.generate(answers.length, (index) => index);
      indices.shuffle();
      _answerMappings[i] = indices;

      // Update question with shuffled answers
      _shuffledQuestions[i]['answers'] = [
        for (var idx in indices) answers[idx],
      ];
    }
  }
  ...
  Future<void> _submitQuiz() async {
    ...
    if (_userAnswers.length < _shuffledQuestions.length) {
      final unanswered = _shuffledQuestions.length - _userAnswers.length;
    ...
    final score = (correctAnswers / _shuffledQuestions.length * 100).round();
                        ...
                        children: [
                          Row(
                            ...
                            children: [
                              ...
                              Text(
                                '${_userAnswers.length}/${_shuffledQuestions.length} answered',
                                style: const TextStyle(
                                  fontSize: 14,
                                  color: Color(0xFF64748B),
                                ),
